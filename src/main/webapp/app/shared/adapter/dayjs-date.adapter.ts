...
function range(length: number, valueFunction: (index: number) => any): string[] {
  const valuesArray: string[] = Array(length);
...
export class DayjsDateAdapter extends DateAdapter<dayjs.Dayjs> {
  ...
  createDate(year: number, month: number, date: number): dayjs.Dayjs {
    ...
    const result = this.dayJs([year, month, date], undefined, undefined, true).startOf('day').locale(this.locale);
  ...
  override deserialize(value: any): dayjs.Dayjs | null {
    ...
    if (typeof value === 'string') {
      ...
      date = value.includes('T') ? this.dayJs(value) : this.dayJs(value, undefined, undefined, true);
    }
