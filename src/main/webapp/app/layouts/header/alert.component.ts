        ...
        case 500: {
          if (httpErrorResponse.error === null || httpErrorResponse.error === undefined) {
            ...
            if (httpErrorResponse.error !== '' && httpErrorResponse.error.message) {
              ...
              } else if (httpErrorResponse.error.message.toLowerCase().startsWith('error.')) {
                this.addAlert('error', httpErrorResponse.error.message);
              } else {
                this.addAlert('error', 'error.http.500');
              }
        ...
        case 400: {
          ...
          if (errorHeader) {
            ...
            for (const fieldError of fieldErrors) {
              const applicationName: string = fieldError.applicationName;
              const errorType: string = fieldError.type;

              let messageToTranslate: string;

              if (fieldError.field) {
                const convertedField: string = fieldError.field.replace(/\[\d*]/g, '[]');
                messageToTranslate = applicationName
                  .concat('.')
                  .concat(fieldError.objectName)
                  .concat('.')
                  .concat(convertedField)
                  .concat('.')
                  .concat(errorType);
              } else {
                messageToTranslate = applicationName.concat('.').concat(fieldError.objectName).concat('.').concat(errorType);
              }

              this.addAlert('error', messageToTranslate);
            }
