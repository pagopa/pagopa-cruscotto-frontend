...
type PagopaData = Omit<IB4PagoPaDrilldown, 'partnerFiscalCode' | 'dataDate'> & {
...
export class KpiB4PagopaDataDrilldownService {
  ...
  constructor() {
    this.resourceUrl = this.applicationConfigService.getEndpointFor('api/kpi-b4-pagopa-data');
  }

  /**
   * NUOVO: recupera i record di drilldown per l'analyticDataId selezionato
   */
  findByAnalyticDataId(analyticDataId: number): Observable<IB4PagoPaDrilldown[]> {
  ...
  private convertFromServer(p: PagopaData): IB4PagoPaDrilldown {
    return {
