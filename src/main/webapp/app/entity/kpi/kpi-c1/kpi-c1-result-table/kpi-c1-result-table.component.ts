...
import { KpiC1ResultService } from '../service/kpi-c1-result.service';
import { KpiC1Result, OutcomeStatus } from '../models/KpiC1Result';
...
@Component({
  selector: 'jhi-kpi-c1-result-table',
  standalone: true,
  templateUrl: './kpi-c1-result-table.component.html',
  styleUrls: ['./kpi-c1-result-table.component.scss'],
  imports: [
    CommonModule,
    MatPaginator,
    MatTableModule,
...
export class KpiC1ResultTableComponent implements AfterViewInit, OnChanges, OnInit {
  ...
  private readonly kpiC1ResultService = inject(KpiC1ResultService);
  ...
  ngOnChanges(): void {
    if (this.moduleId) {
      this.fetchKpiC1Results(this.moduleId);
    }
  }
  ...
  /**
   * Recupera i risultati KPI C1 dall'API
   */
  fetchKpiC1Results(moduleId: number): void {
    this.spinner.show('isLoadingResultsKpiC1ResultTable').then(() => {
      this.isLoadingResults = true; // Indica che il caricamento è in corso

      this.kpiC1ResultService.getKpiResults(moduleId).subscribe({
        next: (data: KpiC1Result[]) => this.onSuccess(data),
        error: () => this.onError(),
      });
    });
  }
   ...
   */
  protected onSuccess(data: KpiC1Result[]): void {
    this.spinner.hide('isLoadingResultsKpiC1ResultTable').then(() => {
  ...
  protected onError(): void {
    this.spinner.hide('isLoadingResultsKpiC1ResultTable').then(() => {
      ...
      console.error('Errore durante il recupero dei risultati KPI C1');
    });
  }
   ...
   */
  emitShowDetails(kpiC1ResultId: number): void {
    if (this.selectedElementId === kpiC1ResultId) {
      // Se l'elemento è già selezionato, deseleziona
      this.selectedElementId = null;
    } else {
      // Altrimenti seleziona l'elemento
      this.selectedElementId = kpiC1ResultId;
    }
    this.showDetails.emit(kpiC1ResultId);
  }
}
