...
import { KpiB8DetailResultService } from '../service/kpi-b8-detail-result.service';
import { EvaluationType, KpiB8DetailResult } from '../models/KpiB8DetailResult';
...
@Component({
  ...
  templateUrl: './kpi-b8-detail-result-table.component.html',
  styleUrls: ['./kpi-b8-detail-result-table.component.scss'],
  imports: [
    MatPaginatorModule,
    MatSortModule,
    MatTableModule,
...
export class KpiB8DetailResultTableComponent implements AfterViewInit, OnChanges, OnInit {
  ...
  @Input() kpiB8ResultId: number | undefined;
  ...
  private readonly kpiB8DetailResultService = inject(KpiB8DetailResultService);
  ...
  ngOnChanges(): void {
    if (this.kpiB8ResultId) {
      this.fetchKpiB8DetailResults(this.kpiB8ResultId);
    }
  }
  ...
  /**
   * Fetch Kpi B8 Detail Results by kpiB8ResultId
   */
  fetchKpiB8DetailResults(kpiB8ResultId: number): void {
    this.spinner.show('isLoadingResultsKpiB8DetailResultTable').then(() => {
      this.isLoadingResults = true;
      this.kpiB8DetailResultService.findByResultId(kpiB8ResultId).subscribe({
        next: (data: KpiB8DetailResult[]) => this.onSuccess(data),
        error: () => this.onError(),
      });
    });
  }
   ...
   */
  protected onSuccess(data: KpiB8DetailResult[]): void {
    this.spinner.hide('isLoadingResultsKpiB8DetailResultTable').then(() => {
  ...
  protected onError(): void {
    this.spinner.hide('isLoadingResultsKpiB8DetailResultTable').then(() => {
      ...
      console.error('Errore durante il recupero dei dettagli KPI B8');
    });
  }
   ...
   */
  emitShowDetails(kpiB8DetailResultId: number): void {
    if (this.selectedElementId === kpiB8DetailResultId) {
      // Se l'elemento è già selezionato, deseleziona
      this.selectedElementId = null;
    } else {
      // Altrimenti seleziona l'elemento
      this.selectedElementId = kpiB8DetailResultId;
    }
    this.showDetails.emit(kpiB8DetailResultId);
  }
}
