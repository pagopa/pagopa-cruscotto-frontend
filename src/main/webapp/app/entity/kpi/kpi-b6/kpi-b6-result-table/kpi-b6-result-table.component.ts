...
import { KpiB6ResultService } from '../service/kpi-b6-result.service';
import { KpiB6Result, OutcomeStatus } from '../models/KpiB6Result';
...
@Component({
  selector: 'jhi-kpi-b6-result-table',
  standalone: true,
  templateUrl: './kpi-b6-result-table.component.html',
  styleUrls: ['./kpi-b6-result-table.component.scss'],
  imports: [
    CommonModule,
    MatPaginator,
    MatTableModule,
...
export class KpiB6ResultTableComponent implements AfterViewInit, OnChanges, OnInit {
  ...
  private readonly kpiB6ResultService = inject(KpiB6ResultService);
  ...
  ngOnChanges(): void {
    if (this.moduleId) {
      this.fetchKpiB6Results(this.moduleId);
    }
  }
  ...
  /**
   * Recupera i risultati KPI B6 dall'API
   */
  fetchKpiB6Results(moduleId: number): void {
    this.spinner.show('isLoadingResultsKpiB6ResultTable').then(() => {
      this.isLoadingResults = true; // Indica che il caricamento è in corso

      this.kpiB6ResultService.getKpiResults(moduleId).subscribe({
        next: (data: KpiB6Result[]) => this.onSuccess(data),
        error: () => this.onError(),
      });
    });
  }
   ...
   */
  protected onSuccess(data: KpiB6Result[]): void {
    this.spinner.hide('isLoadingResultsKpiB6ResultTable').then(() => {
  ...
  protected onError(): void {
    this.spinner.hide('isLoadingResultsKpiB6ResultTable').then(() => {
      ...
      console.error('Errore durante il recupero dei risultati KPI B6');
    });
  }
   ...
   */
  emitShowDetails(kpiB6ResultId: number): void {
    if (this.selectedElementId === kpiB6ResultId) {
      // Se l'elemento è già selezionato, deseleziona
      this.selectedElementId = null;
    } else {
      // Altrimenti seleziona l'elemento
      this.selectedElementId = kpiB6ResultId;
    }
    this.showDetails.emit(kpiB6ResultId);
  }
}
